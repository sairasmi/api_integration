...
def getBlockBorewellPendingData(fyear,schemename):
    ...
    """
    select_query = 'SELECT vch_lgd_code FROM t_district_borewell_pendingapplication_data where deleted_at is NULL AND vch_finacial_year ="{0}" AND vch_scheme_name="{1}" '.format(fyear,schemename)
    cur = mc.conn.cursor()
        ...
        for rdata in  response_json:
            ...
            try:            
                ...
                mc.conn.commit()
                    ...
                    mc.conn.rollback()
                    logging.error("Error in insertion - t_block_borewell_pendingapplication_data")
                    logging.error("Error in getting data for t_block_borewell_pendingapplication_data from API getReportForAdapForPendingWithAAOBlockWise")
                    logging.error(e)
    print("Block Bore well data completed")
if __name__ == "__main__":
    stm = "TRUNCATE TABLE t_block_borewell_pendingapplication_data"
    cur = mc.conn.cursor()
    cur.execute(stm)
