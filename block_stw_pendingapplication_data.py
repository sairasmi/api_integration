...
def getBlockSTWPendingData(fyear):
    ...
    for lgd_code in dist_data:
        ...
        for rdata in  response_json:
            ...
            try:            
                insert_query = "INSERT IGNORE INTO t_block_stw_pendingapplication_data (vch_dist_lgd_code,vch_block_name,vch_aao_initial_pending,vch_goahead_pending,vch_finacial_year,created_at,updated_at) VALUES (%s, %s, %s, %s, %s,%s,%s)"
                valdata =(lgd_code,block_name,aao_initial_pending,goahead_pending,fyear,current_datetime,current_datetime)
                cur = mc.conn.cursor()
